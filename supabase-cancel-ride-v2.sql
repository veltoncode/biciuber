-- ==============================================================================
-- BICIUBER - CANCELAMENTO REAL DE CORRIDA PELO PASSAGEIRO V2 (ETAPA 6)
-- ==============================================================================
-- Finalidade:
--   Permite que o passageiro cancele uma solicitação de corrida ativa própria.
--   V2: Restaura a disponibilidade do motorista (is_available = true) caso
--   um motorista já estivesse atribuído à corrida no momento do cancelamento.
-- ==============================================================================
...
DECLARE
    ...
    v_driver_id UUID;
BEGIN
    -- 1. Buscar status atual da corrida e motorista, verificar se existe e token confere
    SELECT R.status, R.driver_id INTO v_current_status, v_driver_id
    FROM public.rides R
    ...
    -- 3. Restaura disponibilidade do motorista se havia um motorista associado
    ...
    -- 5. Garantia final caso concorrência altere o status antes do UPDATE
    IF NOT FOUND THEN
